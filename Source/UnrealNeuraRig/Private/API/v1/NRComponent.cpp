...
void UNRComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (Raw.Num() > 0)
	{
		...
		{
			FVector dFootScale = FVector(RigScales.MaxFootStrideX, RigScales.MaxFootWidthY, RigScales.MaxFootHeightZ);
			FVector dLocalPosR = FVector(dPacketRecive[0].X - RigScales.FootOffsetX, dPacketRecive[0].Y - RigScales.FootOffsetY, dPacketRecive[0].Z - RigScales.FootOffsetZ) * dFootScale;
			FVector dLocalPosL = FVector(dPacketRecive[2].X - RigScales.FootOffsetX, dPacketRecive[2].Y - RigScales.FootOffsetY, dPacketRecive[2].Z - RigScales.FootOffsetZ) * dFootScale;
		...
		if (PacketRecive.Num() == 20)
		{
			...
			// LegIK rotate
			float TargetCalfPitchR = PacketRecive[9].X * RigScales.MaxThighPitch; // Calf
			float TargetCalfPitchL  = PacketRecive[11].X * RigScales.MaxCalfPitch;
			
			float TargetThighPitchR = PacketRecive[13].X * RigScales.MaxCalfPitch; // Thigh
